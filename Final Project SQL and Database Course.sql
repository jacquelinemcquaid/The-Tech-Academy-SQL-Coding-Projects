...
CREATE TABLE BOOK_COPIES (
	...
	--Number of Copies from each branch--
...
Name
FROM BORROWER
LEFT JOIN BOOK_LOANS ON BORROWER = CardNo
WHERE NOT EXISTS (SELECT NULL FROM BOOK_LOANS WHERE CardNo = BORROWER)
;
...
INNER JOIN BOOKS ON BOOKS = BookID
INNER JOIN LIBRARY_BRANCH ON LIBRARY_BRANCH = BranchID
WHERE DateDue='October 15, 2020' AND BranchID='2789'
;
--End Test--
...
AS
SELECT BranchName, COUNT(DateDue) as BooksLoaned
FROM BOOK_LOANS
INNER JOIN LIBRARY_BRANCH ON LIBRARY_BRANCH = BranchID
GROUP BY BranchName
;
